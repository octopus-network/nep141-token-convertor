...
impl StorageManagement for TokenConvertor {
    ...
    fn storage_deposit(
        ...
        let mut account = self.internal_get_account(&account_id).unwrap_or(Account::new());
        assert!(already_registered && attach_amount >= min_balance, "ERR_DEPOSIT_LESS_THAN_REGISTER_NEED.");
    ...
    fn storage_withdraw(&mut self, mut amount: Option<U128>) -> StorageBalance {
        assert_one_yocto();
        let transfer_amount: u128 = self.internal_use_account(&env::predecessor_account_id(),true, |account| {
            let withdraw_amount = amount.map(|e|e.0).unwrap_or(account.storage_available_balance()) ;
        ...
        return self.storage_balance_of(env::predecessor_account_id())
            .unwrap();
    ...
    fn storage_unregister(&mut self, force: Option<bool>) -> bool {
        ...
        if let Some(account) = self.internal_get_account(&account_id) {
            ...
            assert!(account.tokens.is_empty(),"ERR_STORAGE_UNREGISTER_TOKENS_NOT_EMPTY");
