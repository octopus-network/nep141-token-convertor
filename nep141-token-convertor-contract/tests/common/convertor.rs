...
impl ConvertorContract {
    ...
    pub async fn deploy(
        ...
        deploy_account.deploy(worker, CONVERTOR_WASM_BYTES).await.unwrap();
        deploy_account
            .call(worker, deploy_account.id(), "new")
            .args_json((owner_account.id().clone(),parse_near!("1 N").to_string())).unwrap()
            .transact().await.unwrap();
        ConvertorContract {contract_id: deploy_account.id().clone() }
    }

    pub async fn get_account(&self, worker: &Worker<Sandbox>, account_id: AccountId) -> AccountView {
        worker.view(
            &self.contract_id,
            "get_account",
    ...
    pub async fn get_storage_fee_gap_of(&self, worker: &Worker<Sandbox>,account_id: &workspaces::AccountId) -> U128 {
        worker.view(
            &self.contract_id,
    ...
    pub async fn get_pools(&self, worker: &Worker<Sandbox>,from_index: u32, limit: u32) -> Vec<ConversionPool> {
        worker.view(
            &self.contract_id,
            "get_pools",
    ...
    pub async fn register_account(&self, worker: &Worker<Sandbox>, signer: &Account) -> anyhow::Result<CallExecutionDetails> {
    ...
    pub async fn delete_pool(&self,worker: &Worker<Sandbox>, signer: &Account, pool_id: PoolId) -> anyhow::Result<CallExecutionDetails> {
        signer
